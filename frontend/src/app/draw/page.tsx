...
export default function Draw() {
  ...
  const handleSelectShape = useCallback((item: CarouselClickItem) => {
    ...
    console.log(
      `選択: ${item.description}. 状態を更新しました。(localStorageには保存しません)`
    );
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedData) {
        ...
        const isHeart =
          JSON.stringify(heartShape) === JSON.stringify(parsedPoints);
        const isStar =
          JSON.stringify(starShape) === JSON.stringify(parsedPoints);
        const isCircle =
          JSON.stringify(circleShape) === JSON.stringify(parsedPoints);
  ...
  const isClearButtonDisabled =
    userDrawnPoints.length === 0 && selectedShape === null;
